    ...
    if (check == 1) {
      if (parseInt(creditBox.innerHTML) - parseInt(val.innerHTML) < 0) {
        ...
        <div class="col text-center">
          <img src="${e.target.parentElement.parentElement.children[0].children[0].src}" alt="" class="img-fluid">
          <div class="text-white  mx-auto">${e.target.parentElement.parentElement.children[3].innerHTML}</div>
        </div>
        `;
        playerDiv.appendChild(ele);
        apensec(e);
...
function apensec(e){
  let ele2 = document.createElement('div');
        ...
        `;
        playerdiv2.appendChild(ele2);
}
function remsec(e){
  let i = parseInt(playerBox.innerHTML);
  for(let j=0;j<=i;j++){
    let runn = playerDiv.children[j].children[0].children[0].src;
    if(runn == (e.target.parentElement.parentElement.children[0].children[0].src)){
      playerDiv.removeChild(playerDiv.children[j]);
      playerdiv2.removeChild(playerdiv2.children[j]);
    }
  }      
}